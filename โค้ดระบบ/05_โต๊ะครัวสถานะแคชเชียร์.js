        ...
        function formatItemNameHTML(name, isCanceledOrDone = false) {
            ...
            if (match) {
                ...
                let html = `<b class="${lineClass} text-slate-800 font-bold">${baseName}</b>`;
                subItems.forEach(sub => {
                    html += `<br><span class="text-[11px] text-slate-500 pl-3 inline-block font-semibold ${lineClass}">↳ ${sub}</span>`;
                });
                return html;
            }
            return `<span class="${lineClass}">${name}</span>`;
        }
