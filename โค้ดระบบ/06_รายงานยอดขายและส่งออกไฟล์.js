        ...
        async function fetchSalesData() { 
            try { 
                ...
                let targetDateKey = dateInputVal || getBusinessDateKey();
                
                let res = await fetch(`${FIREBASE_URL}OrderHistory/${targetDateKey}.json`);
                let data = await res.json();
                let isLegacy = false;
                
                if(!data || Object.keys(data).length === 0) {
                    res = await fetch(`${FIREBASE_URL}Orders.json`);
                    data = await res.json() || {};
                    isLegacy = true;
                }
                ...
                for(let key in data) { 
                    ...
                    if(order && order.status === 'paid') { 
                        ...
                        const isMatch = !isLegacy || (orderBizDateStr === targetDateStr);
                        if (isMatch) { 
