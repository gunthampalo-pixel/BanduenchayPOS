        ...
        function addToCart(name, price, note, category, qty = 1, menuKey = '') { const existing = cart.find(item => item.name === name && item.note === note); if(existing) { existing.qty += qty; existing.totalPrice = existing.qty * price; } else { cart.push({ name, price: Number(price), qty: qty, totalPrice: qty * Number(price), note: note, itemStatus: 'pending', category: category, menuKey: menuKey }); } updateCartUI(); const cartCountEl = document.getElementById('cartCount'); cartCountEl.classList.add('scale-150'); setTimeout(() => cartCountEl.classList.remove('scale-150'), 200); }
